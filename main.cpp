...
void semi(){
	...
	for(int i = 0; i < strlen(ele); i++) {
			glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, ele[i]);	
	}
	glColor3f(0.30f, 0.30f, 1.00f);
	glRasterPos3f(30, 40, 0);
	char hole[] = "Holes";
	for(int i = 0; i < strlen(hole); i++) {
			glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, hole[i]);	
	}	
	//electrons
	glColor3f(0.0,0.0,1.0);
	glPointSize(5);
